...
macro_rules! assert_readable_by_polling {
    ($stream:expr) => {
        assert_ok!(poll_fn(|cx| $stream.readable().poll_unpin(cx)).await);
    };
}
...
macro_rules! assert_not_readable_by_polling {
    ($stream:expr) => {
        poll_fn(|cx| {
            assert_pending!($stream.readable().poll_unpin(cx));
            Poll::Ready(())
        })
        .await;
    };
...
macro_rules! assert_writable_by_polling {
    ($stream:expr) => {
        assert_ok!(poll_fn(|cx| $stream.writable().poll_unpin(cx)).await);
    };
}
...
macro_rules! assert_not_writable_by_polling {
    ($stream:expr) => {
        poll_fn(|cx| {
            assert_pending!($stream.writable().poll_unpin(cx));
            Poll::Ready(())
        })
        .await;
    };
