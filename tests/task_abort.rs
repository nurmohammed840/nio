...
use nio::runtime::Runtime;
use nio::{runtime::Builder, time};
...
async fn test_abort_without_panic_3157() {
    // let handle = nio::spawn(async move { time::sleep(Duration::new(100, 0)).await });
    let handle = nio::spawn(async move { pending::<()>().await });
...
async fn test_join_error_display() {
    // `String` payload
    let join_err = nio::spawn(async move {
        let value = 1234;
    ...
    // `&'static str` payload
    let join_err = nio::spawn(async move { panic!("Const payload") })
        .await
        .unwrap_err();
    ...
    // Non-string payload
    let join_err = nio::spawn(async move { std::panic::panic_any(1234i32) })
        .await
        .unwrap_err();
